...
if [ ! -d curl ]; then
    ...
    cmake \
        ...
        -DBUILD_TESTING=Off \
        -DBUILD_CURL_EXE=Off \
        -DZLIB_LIBRARY=/usr/local/aarch64-linux-gnu/lib/libz.a \
        -DOPENSSL_SSL_LIBRARY=/usr/local/aarch64-linux-gnu/lib/libssl.a \
        -DOPENSSL_CRYPTO_LIBRARY=/usr/local/aarch64-linux-gnu/lib/libcrypto.a \
        -DCMAKE_TOOLCHAIN_FILE=/usr/local/aarch64-linux-gnu/lib/cmake/arm64-toolchain.cmake \
        -DCMAKE_INSTALL_PREFIX=/usr/local/aarch64-linux-gnu \
        ..
    cd ../..

...
def get_latest_ami(region, arch):
    ...
    while True:
        result = ec2_client.describe_images(
            ...
            Filters=[
                {
                    "Name": "name",
                    "Values": [f"ubuntu/images/hvm-ssd/ubuntu-{ubuntu_version}-{arch}-server-*"],
                },
        ...
        if len(result["Images"]) == 0:
            raise Exception(f"No AMIs found for {ubuntu_version} {arch} in {region}")
    ...
    print(
        f"  Latest {ubuntu_version} AMI for {arch} in {region}: "
        f"{images[0]['ImageId']} ({images[0]['CreationDate']})"
    )
    return images[0]["ImageId"]


def update_amis(cfn_filename, mapping, arch):
    print(f"Updating {cfn_filename} for {arch}...")
    output = ""
    mapping_found = False
    in_mapping = False
    ami_id_line = ""
        ...
        for line in fp:
            if line == f"  {mapping}:\n":
                ...
                if not region_match:
                    ...
                    ami_id_line = "      AMIID: " + get_latest_ami(region, arch) + "\n"
            output += line
...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--arch", metavar="ARCH", required=True, help="CPU architecture")
    args = parser.parse_args()
    update_amis(args.file, args.mapping, args.arch)

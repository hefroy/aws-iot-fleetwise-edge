...
aws s3 cp config/creds.json ${S3_URL}
S3_PRESIGNED_URL=`aws s3 presign ${S3_URL} --expires-in ${S3_PRESIGNED_URL_EXPIRY}`
...
echo
echo "Optional: After you have downloaded and scanned the QR code, you can delete the credentials"
echo "for security by running the following command:"
echo "aws s3 rm ${S3_URL}"
